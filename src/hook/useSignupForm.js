...
import { signupCustomer, requestEmailCode, verifyEmailCode } from "../api/customers";
...
export function useSignupForm() {
  ...
  const handleVerifyCode = async () => {
    ...
    setVerifying(true);
    const res = await verifyEmailCode({ email, code: emailConfirm.trim() });
  ...
  const handleSignup = async () => {
    ...
    setLoading(true);
    const res = await signupCustomer({ username, email, password, passwordConfirm });
console.log(res);
    if (res.ok) {
      setMsg(res.data?.email ? `가입 완료! 이메일: ${res.data.email} (ID: ${res.data.customerId})` : "가입이 완료되었어요.");
    } else {
      setErrMsg(res.message);
    }
